...
class NotesService {
  constructor(collaborationsService, cacheService) {
    ...
    this._cacheServie = cacheService;
  }
  ...
  async addNote({ title, body, tags, owner }) {
    ...
    await this._cacheServie.delete(`notes:${owner}`);
    return result.rows[0].id;
  }

  async getNotes(owner) {
    try {
      const result = await this._cacheServie.get(`notes:${owner}`);
